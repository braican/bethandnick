...
(function($) {
  ...
  $(document).ready(function() {
    $('.js-add-new-guest').on('click', function() {
      $(this)
        .closest('#view-event')
        .addClass('adding-new-guest');
    });

    $('.js-import-guests').on('click', function() {
      $(this)
        .closest('#view-event')
        .addClass('importing-guests');
    });
    $('.js-close-import-modal').on('click', function() {
      $(this)
        .closest('#view-event')
        .removeClass('importing-guests');
    });

    $('.js-cancel-add-new-guest').on('click', function() {
      $(this)
        .closest('#view-event')
        .removeClass('adding-new-guest');
    });

    $('.js-add-another-guest').on('click', function() {
      var $sourceInput = $('#gl-guest-name').parent();
      var $newInput = $sourceInput.clone();
      var $remove = $('<button class="button button-remove" type="button">X</button>').appendTo(
        $newInput,
      );
    ...
    $('.js-edit-guest-attending-form').on('submit', function(event) {
      ...
      $(this)
        .closest('td')
        .removeClass('gl-editing-cell')
        .data('value', isAttending)
        .find('.js-guest-value')
        .text(isAttending > 0 ? 'Yes' : 'No');
    ...
    $('.js-import-form').on('submit', function(event) {
      event.preventDefault();
      var data = new FormData(this);
      $(this).addClass('loading');

      $.ajax({
        url: data.get('ajax_url'),
        type: 'POST',
        data: data,
        cache: false,
        contentType: false,
